...
class AlertPromptService(BasePromptService):
    """Prompt service specialized for alert analysis."""

    def get_system_prompt_config(self) -> dict[str, str]:
        """Get alert analysis system prompt configuration."""
        return self.config.get_alarm_system_prompt_config()

    def build_prompt(self, session_id: str, messages: str, msg_count: int = 0) -> list[dict[str, str]]:
        """Build prompt specifically for alert analysis."""
