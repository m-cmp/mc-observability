...
class AnomalyHistoryService:
    ...
    def _build_body(self):
        field_mapping = {"cpu": "usage_idle", "mem": "used_percent"}
        ...
        return {
            "conditions": [{"key": "ns_id", "value": self.path_params.nsId}, {"key": "target_id", "value": self.path_params.targetId}],
    ...
    def create_res_data(self, results, raw_data):
        ...
        for entry in results:
            ...
            value = AnomalyDetectionHistoryValue(
                timestamp=entry["timestamp"], anomaly_score=entry.get("anomaly_score"), is_anomaly=entry.get("isAnomaly"), value=resource_pct_value
            )
            values.append(value)

        data = AnomalyDetectionHistoryResponse(ns_id=self.path_params.nsId, target_id=self.path_params.targetId, measurement=self.query_params.measurement.value, values=values)
