...
class AnomalySettingsService:
    ...
    def create_setting(self, setting_data: dict) -> ResBodyVoid | JSONResponse:
        ...
        if duplicate:
            return JSONResponse(
                status_code=409,
                content={"rs_code": "409", "rs_msg": "A record with the same id and measurement already exists."},
            )
    ...
    def update_setting(self, setting_seq: int, update_data: dict) -> ResBodyVoid | JSONResponse:
        update_data = {
            key.upper(): (value.value if isinstance(value, Enum) else value) for key, value in update_data.items()
        }
