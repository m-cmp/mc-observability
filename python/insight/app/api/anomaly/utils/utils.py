...
class AnomalyService:
    ...
    def _build_body(setting: object):
        ...
        return {
            "conditions": [{"key": "ns_id", "value": setting.NAMESPACE_ID}, {"key": "target_id", "value": setting.TARGET_ID}],
    ...
    def cpu_percent_change(df: pd.DataFrame) -> pd.DataFrame:
        df.loc[df["resource_id"].str.contains("cpu"), "resource_pct"] = df.loc[df["resource_id"].str.contains("cpu"), "resource_pct"].apply(
            lambda x: 100 - x if not pd.isna(x) else np.nan
        )
...
class AnomalyDetector:
    ...
    def run_rrcf(self, df, shingle_size: int):
        ...
        results = pd.DataFrame({"timestamp": df["timestamp"], "anomaly_score": complete_scores.round(4), "isAnomaly": anomalies.astype(int)})
