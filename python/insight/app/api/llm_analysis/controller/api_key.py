...
from app.api.llm_analysis.request.req import DeleteAPIKeyFilter, GetAPIKeyFilter, PostAPIKeyBody
from app.api.llm_analysis.response.res import ResBodyLLMAPIKey, ResBodyLLMAPIKeys
...
async def get_llm_api_key(db: Session = Depends(get_db), query_params: GetAPIKeyFilter = Depends()):
...
async def delete_llm_api_key(db: Session = Depends(get_db), query_params: DeleteAPIKeyFilter = Depends()):
    service = CommonAPIKeyService(db=db)
