...
from app.api.prediction.request.req import GetHistoryPath, GetPredictionHistoryQuery, PredictionBody, PredictionPath
...
class PredictionService:
    ...
    def _build_body(self, nsId: str, targetId: str, target_type: str, measurement: str, prediction_range: str):
        target_mapping = {"vm": "target_id", "mci": "mci_id"}
        ...
        return {
            "conditions": [{"key": "ns_id", "value": nsId}, {"key": target_value, "value": targetId}],
    ...
    def get_data(self, path_params: PredictionPath, body_params: PredictionBody):
        ...
        url = self._build_url("influxdb/metric")
    ...
    def get_prediction_history(self, path_params: GetHistoryPath, query_params: GetPredictionHistoryQuery):
        prediction_points = self.influxdb_repo.query_prediction_history(
            nsId=path_params.nsId, targetId=path_params.targetId, measurement=query_params.measurement, start_time=query_params.start_time, end_time=query_params.end_time
        )
        ...
        filtered_values = [v for v in values if start_time_dt <= datetime.strptime(v["timestamp"], "%Y-%m-%dT%H:%M:%SZ") <= end_time_dt]
