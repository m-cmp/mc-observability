...
from app.api.prediction.description.description import (
...
from app.api.prediction.request.req import GetHistoryPath, GetMeasurementPath, GetPredictionHistoryQuery, PredictionBody, PredictionPath
from app.api.prediction.response.res import (
    PredictionHistory,
    PredictionOptions,
    PredictionResult,
...
async def predict_monitoring_data(body_params: PredictionBody, path_params: PredictionPath = Depends()):
    ...
    prediction_result = PredictionResult(
        ns_id=path_params.nsId, target_id=path_params.targetId, measurement=body_params.measurement, target_type=body_params.target_type, values=result_dict
    )
...
async def get_prediction_history(path_params: GetHistoryPath = Depends(), query_params: GetPredictionHistoryQuery = Depends()):
    prediction_service = PredictionService()
    result_dict = prediction_service.get_prediction_history(path_params, query_params)

    prediction_history = PredictionHistory(ns_id=path_params.nsId, target_id=path_params.targetId, measurement=query_params.measurement, values=result_dict)
