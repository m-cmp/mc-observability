...
from config import DB_HOST, DB_PORT, DB_USER, DB_PASSWORD, DB_NAME, MCP_READ_ONLY, MCP_MAX_POOL_SIZE, EMBEDDING_PROVIDER, logger
...
class MariaDBServer:
    ...
    async def initialize_pool(self):
        ...
        try:
            ...
            self.pool = await asyncmy.create_pool(
                host=DB_HOST, port=DB_PORT, user=DB_USER, password=DB_PASSWORD, db=DB_NAME, minsize=1, maxsize=MCP_MAX_POOL_SIZE, autocommit=self.autocommit, pool_recycle=3600
            )
    ...
    async def create_vector_store_tool(
        self, database_name: str, vector_store_name: str, embedding_service: EmbeddingService, model_name: Optional[str] = None, distance_function: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if await self._table_exists(database_name, vector_store_name):
            ...
            return {"status": "exists", "message": message, "database_name": database_name, "vector_store_name": vector_store_name}
        ...
        try:
            ...
            return {"status": "success", "message": success_message, "database_name": database_name, "vector_store_name": vector_store_name}
        except Exception as e:
    ...
    async def delete_vector_store(self, database_name: str, vector_store_name: str) -> Dict[str, Any]:
        ...
        if not await self._is_vector_store(database_name, vector_store_name):
            message = (
                f"Table '{vector_store_name}' in database '{database_name}' is not a valid vector store (missing indexed 'embedding' column of type VECTOR). Deletion aborted."
            )
            logger.warning(message)
        ...
        try:
            ...
            return {"status": "error", "message": f"{error_message} Reason: {str(e)}", "database_name": database_name, "vector_store_name": vector_store_name}
    ...
    async def run_async_server(self, transport="stdio", host="127.0.0.1", port=8001):
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--transport", type=str, default="stdio", choices=["stdio", "sse"], help="MCP transport protocol (stdio or sse)")
    ...
    try:
        ...
        anyio.run(partial(server.run_async_server, transport=args.transport, host=args.host, port=args.port))
