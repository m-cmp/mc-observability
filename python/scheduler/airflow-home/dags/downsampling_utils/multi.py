...
class DataProcessor:
        ...
        for metirc_info in self.metric_info_list:
            ...
            for field in metirc_info["fields"]:
                ...
                metric_data = self.load_metric_data(
                    measurement=measurement, field=field["field_key"]
                )
                if not metric_data:
                    print("data empty")
                    continue
                ...
                for metric_entry in metric_data:
                    try:
                        ...
                        self.save_to_influx(
                            measurement, result_df, tags=tags, field=field["field_key"]
                        )
                    except Exception as e_msg:
                        print(
                            f"Error processing metric data for {measurement}, field: {field['field_key']}. Error: {e_msg}"
                        )
    ...
    def load_metric_data(self, measurement: str, field: str):
        body = {
            "conditions": [],
            "fields": [{"function": "mean", "field": field}],
            "group_by": ["ns_id", "mci_id", "target_id"],
            "group_time": "1m",
            "measurement": measurement,
            "range": "1h",
        }

        api_url = self.api_base_url + "/monitoring/influxdb/metric"
        metric_data = (
