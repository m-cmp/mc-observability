...
CREATE TABLE IF NOT EXISTS `mc_observability`.`m_cmp_agent_host` (
...
CREATE TABLE IF NOT EXISTS `mc_observability`.`m_cmp_agent_host_item` (
  `SEQ` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '고유번호',
...
CREATE TABLE IF NOT EXISTS `mc_observability`.`m_cmp_agent_host_storage` (
  `SEQ` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '고유번호',
...
CREATE TABLE IF NOT EXISTS `mc_observability`.`m_cmp_agent_mining_db` (
...
-- 테이블 데이터 mc_observability.m_cmp_agent_mining_db:~1 rows (대략적) 내보내기
...
CREATE TABLE IF NOT EXISTS `mc_observability`.`m_cmp_agent_plugin_def` (
  `SEQ` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '고유번호',
  ...
  `IS_INTERVAL` bit(1) DEFAULT b'1',
  PRIMARY KEY (`SEQ`)
) ENGINE=InnoDB AUTO_INCREMENT=10 DEFAULT CHARSET=utf8mb4 COMMENT='모니터링 항목 정의';

INSERT INTO `mc_observability`.`m_cmp_agent_plugin_def` (`SEQ`, `NAME`, `PLUGIN_ID`, `PLUGIN_TYPE`, `OS`, `IS_INTERVAL`) VALUES
	(1, 'cpu', '[[inputs.cpu]]', 'INPUT', 'LINUX,WINDOWS,MACOS', b'1'),
...
CREATE TABLE IF NOT EXISTS `mc_observability`.`m_cmp_agent_summary_influx` (
