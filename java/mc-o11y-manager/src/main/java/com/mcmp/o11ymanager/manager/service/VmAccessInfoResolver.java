 ...
 *
 * <p>{@code BeylaController}(Linux)와 {@code OtelJavaController}(Windows)가 동일한 IP/credential
 * 해석 로직을 공유하기 위해 별도 service로 추출. OS별 분기는 호출자가 책임지고, 본 클래스는 단순히
 * "Tumblebug Node가 주어졌을 때 Ansible로 보낼 AccessInfoDTO를 만들어준다"는 한 가지 책임을 가짐.
 *
 * <p><b>IP 해석 정책:</b> {@code publicIP} 우선, 비어 있으면 Spider raw API로 보강 시도, 그것도 실패하면
 * {@code privateIP} 사용. v0.12.9 Tumblebug가 OpenStack VM의 multi-IP 일부를 응답에서 누락하는
 * 케이스를 우회.
 *
 * <p><b>인증 정책:</b>
 *
 ...
 *   <li>Windows: {@code node.nodeUserPassword} (응답에 살아 있을 때만; cloud-init 완료 후 마스킹됨)
 *       사용자 이름은 {@code node.nodeUserName}이 채워져 있으면 그 값(예: "cb-user") 그대로, 아니면
...
public class VmAccessInfoResolver {
     ...
     *
     * <p>두 번째 단계는 v0.12.9 Tumblebug가 OpenStack VM의 multi-IP 중 일부만 publicIP로 surface하는
     * 케이스를 풀어주기 위함. Spider raw 응답엔 OpenStack 원본 정보가 들어있다.
     */
     ...
     * Tumblebug 응답의 어느 필드와 매칭되는지 환경마다 다를 수 있어 cspResourceName → uid → name 순서로
     * 시도. 호출 실패는 catch만 하고 null 반환.
     */
