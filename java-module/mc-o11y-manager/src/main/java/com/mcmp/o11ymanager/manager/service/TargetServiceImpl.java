...
import com.mcmp.o11ymanager.manager.exception.target.TargetAgentTaskProcessingException;
...
public class TargetServiceImpl implements TargetService {
    ...
    public TargetDTO get(String nsId, String mciId, String targetId) {
        TargetEntity entity =
                targetJpaRepository
                        .findByNsIdAndMciIdAndTargetId(nsId, mciId, targetId)
                        .orElseThrow(
                                () ->
                                        new ResourceNotExistsException(
                                                ...
                                                "TargetEntity",
                                                nsId + "/" + mciId));
        return TargetDTO.fromEntity(entity);
    }

    @Override
    public List<TargetDTO> getByNsMci(String nsId, String mciId) {
        return targetJpaRepository.findByNsIdAndMciId(nsId, mciId).stream()
                .map(TargetDTO::fromEntity)
                .toList();
    }

    @Override
    public TargetDTO getByNsVm(String nsId, String targetId) {
        TargetEntity e =
                targetJpaRepository
                        .findByNsIdAndTargetId(nsId, targetId)
                        .orElseThrow(
                                () ->
                                        new ResourceNotExistsException(
                                                requestInfo.getRequestId(),
                                                "TargetEntity",
                                                nsId + "/" + targetId));
        return TargetDTO.fromEntity(e);
    }

    @Override
    public List<TargetDTO> list() {
        return targetJpaRepository.findAll().stream().map(TargetDTO::fromEntity).toList();
    }

    @Override
    public TargetDTO post(
            String nsId,
            String mciId,
            String targetId,
            TargetStatus targetStatus,
            TargetRequestDTO dto,
            Long influxSeq) {
        TargetEntity target =
    ...
    public void isIdleMonitoringAgent(String nsId, String mciId, String targetId) {
        ...
        if (target.getMonitoringAgentTaskStatus() != TargetAgentTaskStatus.IDLE) {
            throw new TargetAgentTaskProcessingException(
                    requestInfo.getRequestId(),
                    targetId,
                    "monitoringAgentTask",
    ...
    public void isIdleLogAgent(String nsId, String mciId, String targetId) {
        ...
        if (target.getLogAgentTaskStatus() != TargetAgentTaskStatus.IDLE) {
            throw new TargetAgentTaskProcessingException(
                    requestInfo.getRequestId(), targetId, "로그", target.getLogAgentTaskStatus());
        }
    }
    ...
    public void updateMonitoringAgentTaskStatus(
            String nsId, String mciId, String targetId, TargetAgentTaskStatus status) {
        TargetEntity target =
                targetJpaRepository
                        .findByNsIdAndMciIdAndTargetId(nsId, mciId, targetId)
                        .orElseThrow(
        ...
        log.info(
                "[TargetService] Monitoring Service Status {}: {}",
                target.getTargetMonitoringAgentTaskId(),
                target.getMonitoringAgentTaskStatus());
    }
    ...
    public void updateLogAgentTaskStatus(
        ...
        targetJpaRepository.save(target);
        log.info(
                "[TargetService] Log Service Status {}: {}",
                target.getTargetLogAgentTaskId(),
    ...
    public Long getInfluxId(String nsId, String mciId) {
        TargetEntity t =
                targetJpaRepository
                        .findTop1ByNsIdAndMciIdOrderByTargetIdAsc(nsId, mciId)
