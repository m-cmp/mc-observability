package com.mcmp.o11ymanager.exception.target;
...
public class TargetAgentTaskProcessingException extends BaseException {
    public TargetAgentTaskProcessingException(String requestId, String targetId, String agentTypeName, TargetAgentTaskStatus status) {
        super(
                requestId,
                ErrorCode.AGENT_TASK_IN_PROGRESS,
                String.format("호스트(ID: %s)에서 %s 에이전트 작업이 진행 중입니다. (현재 상태: %s)", targetId, agentTypeName, status)
        );
    }
