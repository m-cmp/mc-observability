...
public class QueryFactory {
    ...
    public static String fluxQL(AlertRuleCreateDto dto) {
        String vmIdName = dto.vmScope() + "_id";

        return switch (dto.resourceType()) {
            case CPU -> cpuQuery(dto, vmIdName);
            case MEMORY -> memoryQuery(dto, vmIdName);
            case DISK -> diskQuery(dto, vmIdName);
        };
    }
     ...
     * @param vmIdName Dynamic vm ID field name based on vm scope
     * @return FluxQL query string for CPU usage monitoring
     */
    private static String cpuQuery(AlertRuleCreateDto dto, String vmIdName) {
        return Flux.from("mc-observability/autogen")
                ...
                .filter(
                        Restrictions.and(
                                ...
                                Restrictions.tag(vmIdName).equal(dto.vmId()),
     ...
     */
    private static String memoryQuery(AlertRuleCreateDto dto, String vmIdName) {
        return Flux.from("mc-observability/autogen")
                .range(-3L, -1L, ChronoUnit.MINUTES)
                .filter(
                        Restrictions.and(
     ...
     */
    private static String diskQuery(AlertRuleCreateDto dto, String vmIdName) {
        return Flux.from("mc-observability/autogen")
                .range(-3L, -1L, ChronoUnit.MINUTES)
                .filter(
                        Restrictions.and(
