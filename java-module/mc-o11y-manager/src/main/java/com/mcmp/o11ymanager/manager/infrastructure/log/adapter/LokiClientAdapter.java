...
public class LokiClientAdapter implements LokiPort {
    ...
    public Label fetchLabelValues(
        ...
        LokiLabelsResponseDto response =
                lokiFeignClient
                        .fetchLabelValues(
                                label, formattedStart, formattedEnd, formattedSince, decodedQuery)
    ...
    public LogVolume fetchLogVolumes(String query, String start, String end, Integer limit) {
        ...
        String transformedQuery =
                String.format("sum by (level) (count_over_time(%s [1h] ))", decodedQuery);
        ...
        LokiVolumeResponseDto response =
                lokiFeignClient
                        .fetchLogVolumes(transformedQuery, formattedStart, formattedEnd, limit)
        ...
        try {
                ...
                if (dateStr.length() > 19) { // 밀리초 또는 타임존 정보 포함
                    formatter =
                            java.time.format.DateTimeFormatter.ofPattern(
                                    "yyyy-MM-dd HH:mm:ss.SSS[ XXX][ Z][ z]");
                } else { // 기본 형식
            ...
            // T가 포함된 ISO 형식 (2025-03-24T17:03:03)
                ...
                if (dateStr.endsWith("Z")) {
                    ...
                    dateTime =
                            java.time.LocalDateTime.parse(dateStr).atZone(java.time.ZoneOffset.UTC);
                }
            }
            // 다른 형식일 경우 일단 원본 반환
            else {
