...
public class SemaphoreService {
    ...
    public void initSemaphore() {
        ...
        try {
            ...
            semaphoreDomainService.initProjectRepository(
                    projectName,
        ...
        try {
            semaphoreDomainService.getProjectRepositoryId(
                    projectName, templateNameAgentConfigUpdate);
        } catch (NoSuchElementException e) {
        ...
        try {
            for (int i = 0; i < SemaphoreDomainService.SEMAPHORE_MAX_PARALLEL_TASKS; i++) {
                semaphoreDomainService.checkProjectTemplate(
                        projectName, templateNameAgentInstall + "_" + (i + 1));
            }
        } catch (NoSuchElementException e) {
            log.info("{} 템플릿 생성중... ⚙️", templateNameAgentInstall);
            List<SurveyVar> surveyVars = createAgentInstallTemplate();
            semaphoreDomainService.initProjectTemplate(
                    projectName,
                    inventoryName,
                    templateNameAgentInstall,
                    PLAYBOOK_FILE_NAME,
        ...
        try {
            for (int i = 0; i < SemaphoreDomainService.SEMAPHORE_MAX_PARALLEL_TASKS; i++) {
                semaphoreDomainService.checkProjectTemplate(
                        projectName, templateNameAgentConfigUpdate + "_" + (i + 1));
            }
        } catch (NoSuchElementException e) {
    ...
    private List<SurveyVar> createTemplateCommon() {
        ...
        SurveyVar vmHost = new SurveyVar();
        vmHost.setValues(new ArrayList<>());
        vmHost.setName("target_host");
        vmHost.setTitle("target_host");
        vmHost.setDescription("");
        vmHost.setType("");
        vmHost.setRequired(true);
        surveyVars.add(vmHost);

        SurveyVar vmPort = new SurveyVar();
        vmPort.setValues(new ArrayList<>());
        vmPort.setName("vm_port");
        vmPort.setTitle("vm_port");
        vmPort.setDescription("");
        vmPort.setType("");
        vmPort.setRequired(true);
        surveyVars.add(vmPort);

        SurveyVar vmUser = new SurveyVar();
        vmUser.setValues(new ArrayList<>());
        vmUser.setName("vm_user");
        vmUser.setTitle("vm_user");
        vmUser.setDescription("");
        vmUser.setType("");
        vmUser.setRequired(true);
        surveyVars.add(vmUser);

        SurveyVar vmSSHKey = new SurveyVar();
        vmSSHKey.setValues(new ArrayList<>());
        vmSSHKey.setName("vm_sshkey");
        vmSSHKey.setTitle("vm_sshkey");
        vmSSHKey.setDescription("");
        vmSSHKey.setType("");
        vmSSHKey.setRequired(true);
        surveyVars.add(vmSSHKey);
